    ...
    <div className="min-h-screen font-sans relative selection:bg-[#A9D87A] selection:text-[#1F2517] overflow-x-hidden flex flex-col" style={{ backgroundColor: colors.bgMain, color: colors.primary }}>
      ...
      <div className="relative z-10 w-full max-w-lg mx-auto flex flex-col flex-1 pb-16 shadow-2xl shadow-black/5 bg-white/40 backdrop-blur-3xl min-h-screen">
        ...
        <div className="px-6 mb-8 mt-2">
          <a 
            href={`https://wa.me/${data.whatsapp}?text=${encodeURIComponent('Hola, me gustaría solicitar un presupuesto de Toldos Al Kamas.')}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full py-5 rounded-[2rem] shadow-xl hover:shadow-2xl transition-all duration-300 font-black text-lg active:scale-95 group overflow-hidden relative"
      ...
      {/* WhatsApp FAB Flotante Global (Con diseño unificado) */}
      <div 
        className={`fixed bottom-6 right-6 z-50 transition-all duration-[600ms] flex flex-col items-end gap-2 ${showWa ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
      >
        <div className="flex items-center gap-2 mr-1">
           <span className="bg-white text-xs font-bold px-3 py-1.5 rounded-2xl shadow-sm border drop-shadow-sm whitespace-nowrap hidden sm:block animate-bounce" style={{ color: colors.primary, borderColor: colors.bgCard }}>
             ¿Hablamos?
           </span>
         </div>
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer"
          className="w-14 h-14 rounded-full flex items-center justify-center shadow-2xl hover:shadow-[0_10px_30px_-5px_var(--accentHover)] hover:-translate-y-1 transition-all group"
          title="Contactar por WhatsApp"
          style={{ backgroundColor: colors.accent, color: '#1A2211' }}
        >
